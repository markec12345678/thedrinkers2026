...
export const metadata: Metadata = {
  title: 'O Nas',
  description: 'Spoznaj zgodbo rock skupine The Drinkers - člani, zgodovina in dosežki.',
};
    ...
    <>
      ...
      <section className="relative h-[60vh] flex items-center justify-center bg-rock-dark">
        ...
        <div className="relative z-10 text-center">
          <h1 className="text-5xl md:text-7xl font-bold text-gradient mb-4">O NAS</h1>
      ...
      <Section background="darker">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-8 mt-16">
            {STATS.map((stat) => (
              <div key={stat.label} className="text-center p-8 bg-crimson/10 rounded-lg border border-crimson/30">
                <div className="text-5xl font-bold text-crimson mb-2">{stat.value}</div>
                <div className="text-text-gray font-bold uppercase tracking-wider">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <Section background="gradient">
        <div className="max-w-7xl mx-auto">
          <h2 className="text-4xl font-bold text-crimson text-center mb-12">ČLANI SKUPINE</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8">
            {bandMembers.map((member) => (
              <div key={member.id} className="card p-6 text-center group hover:-translate-y-2 transition-transform duration-300">
                ...
                <p className="text-crimson font-bold text-lg mb-4">{member.role}</p>
                {member.bio && <p className="text-text-gray text-sm">{member.bio}</p>}
              </div>
            ))}
          </div>
        </div>
      ...
      <Section background="darker">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <p className="text-lg text-text-gray mb-8 max-w-2xl mx-auto">
            Prenesite uradni press kit z visoko ločljivostjo slikami, biografijo in logotipi za medijske namene.
          </p>
          <Button size="lg">
