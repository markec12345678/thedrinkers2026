...
import { dropEntry, limitedDrop } from "@/lib/db/schema";
...
export async function POST(request: NextRequest) {
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (
      session.metadata?.type === "limited_drop" &&
      session.metadata.dropEntryId
    ) {
      try {
        await db
          .update(dropEntry)
          ...
          .where(eq(dropEntry.id, session.metadata.dropEntryId));
      } catch (error) {
        console.error("Failed to confirm drop reservation:", error);
      }
    }
  }
      ...
      try {
        ...
        if (Number.isFinite(quantity) && quantity > 0) {
          ...
          await db
            ...
            .where(eq(limitedDrop.id, session.metadata.dropId));
        }
      } catch (error) {
        console.error("Failed to release expired drop reservation:", error);
      }
    }
  }
