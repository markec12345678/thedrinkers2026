#!/usr/bin/env node
/**
 * 🚀 NEON REST API MIGRATION - AUTHENTICATED
...
async function createTables() {
  console.log("🚀 Creating tables via Neon REST API (Authenticated)...\n");
  ...
  for (const table of tables) {
    try {
      const result = await neonQuery(table.def);
      if (
        result.error &&
        ...
        failed++;
        console.log(
          `❌ ${table.name.padEnd(20)} ${result.error.substring(0, 60)}`,
        );
      } else {
        created++;
  ...
  const seedSQL = `INSERT INTO vip_tier (name, display_name, price, priority) VALUES ('bronze', 'Bronze Fan', '9.99', 1), ('silver', 'Silver Supporter', '19.99', 2), ('gold', 'Gold Member', '49.99', 3), ('platinum', 'Platinum VIP', '99.99', 4) ON CONFLICT (name) DO NOTHING`;
  try {
