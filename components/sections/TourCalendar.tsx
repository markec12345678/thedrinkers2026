...
export function TourCalendar() {
  ...
  const upcomingShows = tourDates.filter(
    (date) => !date.soldOut && new Date(date.date) >= now,
  );
  const pastShows = tourDates.filter((date) => new Date(date.date) < now);
              ...
              {upcomingShows.length === 0 ? (
                <div className="card p-8 text-center">
                  <p className="text-text-gray text-lg">
                    Trenutno ni objavljenih prihajajočih koncertov.
                  </p>
                  <Button className="mt-6" variant="secondary" asChild>
                    <a href="/tour">POGLEJ ARHIV KONCERTOV</a>
                  </Button>
                </div>
              ) : (
              ...
              {pastShows.slice(0, 3).map((show, index) => (
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent p-4 flex flex-col justify-end">
                    ...
                    <p className="text-text-gray text-sm">{show.city} • {formatDate(show.date)}</p>
                  </div>
                </motion.div>
              ))}
            </div>
